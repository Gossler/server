...
class Trashbin {
	...
	private static function copyFilesToUser($sourcePath, $owner, $targetPath, $user, $timestamp) {
		...
		if ($view->file_exists($target)) {
			...
			$query->insert('files_trash')
				->set('id', $query->createNamedParameter($targetFilename))
				->set('timestamp', $query->createNamedParameter($timestamp))
				->set('location', $query->createNamedParameter($targetLocation))
	...
	public static function move2trash($file_path, $ownerOnly = false) {
		...
		if ($moveSuccessful) {
			$query = \OC::$server->getDatabaseConnection()->getQueryBuilder();
			$query->insert('files_trash')
				->set('id', $query->createNamedParameter($filename))
				->set('timestamp', $query->createNamedParameter($timestamp))
				->set('location', $query->createNamedParameter($location))
				->set('user', $query->createNamedParameter($owner));
			$result = $query->execute();
			if (!$result) {
